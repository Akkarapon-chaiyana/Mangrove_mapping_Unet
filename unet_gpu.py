...
binary_dir = 'Data/label_cambodia'
sentinel_dir = 'Data/sentinel_cambodia'
...
# Extract unique IDs from filenames (assuming ID is extracted correctly)
...
# Save Model
# torch.save(model.state_dict(), 'Data/my_unet_model.pth')
...
def run_inference_and_save_tiff(predictions_folder='Data/inference', input_folder= sentinel_dir):
    os.makedirs(predictions_folder, exist_ok=True)

    # Load the model weights and set to evaluation mode
    model.load_state_dict(torch.load('Data/my_unet_model.pth'))
    model.to(device)
    model.eval()
